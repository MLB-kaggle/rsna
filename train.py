...
df['mask_slices (png)'] = df['mask_slices (png)'].str.replace('/kaggle/input/lumbar-spine-segmentation', '/mnt/lumbar', regex=False)
df['image_slices (png)'] = df['image_slices (png)'].str.replace('/kaggle/input/lumbar-spine-segmentation', '/mnt/lumbar', regex=False)
...
def load_img(path):
    ...
    img = np.tile(img[...,np.newaxis], [1, 1, 3]) # gray to rgb
...
def run_training(model, optimizer, scheduler, device, num_epochs):
    ...
    for epoch in tqdm(range(1, num_epochs + 1)): 
        ...
        writer.add_scalar("Jaccard/valid", val_jaccard, epoch)
